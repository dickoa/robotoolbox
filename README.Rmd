...
`robotoolbox` is an R client to access data from [KoboToolbox](https://www.kobotoolbox.org/).
...
The package [`dm`](https://cynkra.github.io/dm/) is used to model such relationship and allow you to safely query and join such linked data for your analysis.
...
`Kobotoolbox` comes with a feature that records all activities related to a form submission in a log file. The audit logging metadata is useful for data quality control, security and workflow management. The `kobo_audit` function allow you to read `Kobotoolbox` audit logs file.
...
We can now get `labels` instead of values for `select_multiple` questions with `robotoolbox`.
Let's show this new feature, with the following example:
...
Now let's set the new `select_multiple_label` to `TRUE` and read again the data.
...
Finally, with the function `set_names_from_varlabel` you can replace all the columns (not just `select_multiple`) by their label
