...
[![Codecov Code Coverage](https://codecov.io/gl/dickoa/robotoolbox/branch/master/graph/badge.svg)](https://app.codecov.io/gl/dickoa/robotoolbox)
...
The following examples will utilize UNHCR `KoboToolbox` server url (https://kobo.unhcr.org/). You can replace this URL with https://kf.kobotoolbox.org/, https://eu.kobotoolbox.org/ or any other `KoboToolbox` server URL you typically use.
...
By default, `kobo_setup` sets a maximum of 1,000 submissions per API request, matching the limit enforced by public KoboToolbox servers since KoboToolbox KPI version 2.026.03 (March 2026). If you use a private server with a higher limit, you can configure it with the `page_size` parameter or by setting the `KOBOTOOLBOX_PAGE_SIZE` environment variable:
...
Alternatively, you can set `KOBOTOOLBOX_PAGE_SIZE` in your `.Renviron`:

```{bash, eval=FALSE, engine="sh"}
KOBOTOOLBOX_PAGE_SIZE=30000
```
...
You can also filter submissions server-side using a MongoDB-style `query`, or select specific `fields` to reduce data transfer:
...
`robotoolbox` uses the R package [`labelled`](https://larmarange.github.io/labelled/) to provide tools to manipulate variable labels and value labels. You can learn more about this here.
...
The package [`dm`](https://dm.cynkra.com) is used to model such relationship and allows you to safely query and join such linked data for your analysis. Learn more about it [here](https://dickoa.gitlab.io/robotoolbox/articles/repeat-group-data.html)
