...
`robotoolbox` allows you to access labels from languages in your form. It uses the [`labelled`](https://cran.r-project.org/package=labelled/vignettes/intro_labelled.html) package to encode variable labels for questions labels and value labels for choices labels.
...
This project uploaded on a `Kobotoolbox` server, is loaded using his `uid`.
...
### Listing availables languages

You can list all availables languages in the project using the `kobo_lang` function.
...
`to_character` is an alternative to `to_factor`. It can also be used if you prefer the type of the column to be `character`.
