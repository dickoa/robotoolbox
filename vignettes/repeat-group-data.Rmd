...
In `Kobotoolbox`, this feature is implemented using a repeat group. It works by inserting the questions you want to repeat inside a `begin_repeat`/`end_repeat` loop. Repeat groups can be nested, making possible to have repeat questions within another repeat instance. It can be illustrated with the following project, and associated form.
...
We can see that the output is not a standard a `data.frame` or `tibble`. It's a listing of each `repeat group` loop in our form.
...
A `dm` object is a `list`. As in any list of `tibble`, you can extract each table (`tibble` or `data.frame`), and analyze it separately.
...
We can push this logic to have the widest table possible through a cascade of joins, starting from a table.
