...
`Kobotoolbox` support questions within roster that can be answered several times. A typical use case is when implementing a household survey, with questions for every member of this household.
...
In `Kobotoolbox` it's implemented using a special type of group. Using repeat group, it's possible to loop through questions using the following form type `begin_repeat` (need to be closed using `end_repeat`). It can illustrated with following project, and associated form.
