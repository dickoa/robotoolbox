...
`Kobotoolbox` support questions within roster that can be grouped and that can be answered repeatedly. A typical use case is when implementing a household survey, with some series of questions for each household members.

## Loading data

In `Kobotoolbox`, this feature is implemented using a repeat group. It works by inserting the questions you want to repeat inside a `begin_repeat`/`end_repeat` loop. It's also to have a nested structure, repeat questions within another repeat instance. It can illustrated with following project, and associated form.
...
The above form was uploaded to the server, it's named `nested_roster`, and can be loaded from the list of asset `al`.

```{r, eval = FALSE}
al <- kobo_asset_list()
uid <- filter(al, name == "nested_roster") |>
  pull(uid)
...
We can see that the output is not a standard a `data.frame` or `tibble`. It's a listing of each repeat loops in our form.
...
A `dm` object is a `list`. As in any list of `tibble`, you can extract each table, and analyze it separately. The principal table, where you have the first repeat groups is named `main`.

```{r access_specific1}
glimpse(df$main)
```

The other tables uses their form names, for example `education`.
...
One major advantage of using the `dm` package, is the ability to dynamically filter tables while keeping the link between them. In this example, filtering the `main` table will propagate to the `education` and `demo` tables. The `hobbies_list` and `lang_list` tables are linked to the `demo` table, and will be filtered too.
...
In some cases, it's simpler to analyze the joined data, `dm_join_to_tbl` can be used for safely joining the data using the structure and links between tables.
...
We can push this logic to have the widest table possible through a cascade of joins.
