...
In order to understand how the data is stored, we can visualize the relationships between tables (repeat groups loop) and the schema of the dataset. The schema can be drawn using the the `dm_draw` function.
...
The `dm` package has a lot of helper functions to manipulate `dm` object. For example, we can get the number of rows of each table using `dm_nrow`
...
A `dm` object is a `list` of `data.frame`. As in any list of `data.frame`, you can extract each table (`data.frame`), and analyze it separately.
...
The other tables are named after the name of the associated repeat group. The `education` table is named after `education` `repeat group`.
...
We can join the `education` table to the `main` table, by using `dm_flatten_to_tbl` and starting from `education`.
...
We can push this logic to have the widest table possible through a cascade of joins from a deeper table (`start` argument) to the `main` table. Using `.start = hobbies_list` as an example, it'll perform two joins.
